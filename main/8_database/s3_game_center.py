...
def change_window(hide_window: Tk, show_window: Tk):
    ...
    if show_window == main_window:
        if access_level == 1:
            btn_view_orders_main_window_manager.grid(
                row=1, column=1, cnf=CNF_BTNS_SIGN_UP_WINDOW_GRID
            )
            btn_view_orders_main_window_user.grid_forget()
            btn_add_update_delete_game_main_window.grid(
                row=2, column=1, cnf=CNF_BTNS_SIGN_UP_WINDOW_GRID
            )
        else:
            btn_view_orders_main_window_manager.grid_forget()
            btn_view_orders_main_window_user.grid(
                row=1, column=1, cnf=CNF_BTNS_SIGN_UP_WINDOW_GRID
            )
        ...
        treev_view_orders_window_manager.delete(
            *treev_view_orders_window_manager.get_children()
        )
        for index, order in enumerate(result):
            treev_view_orders_window_manager.insert(
                "", END, text=order[0], values=(index + 1, *order[1:])
            )
    elif show_window == view_orders_window_user:
        result = connection.get_all_orders_of_user_by_id(user_id)
        treev_view_orders_window_user.delete(
            *treev_view_orders_window_user.get_children()
        )
        for index, order in enumerate(result):
            treev_view_orders_window_user.insert(
                "", END, text=order[0], values=(index + 1, *order[1:])
            )
        ...
        for index, order in enumerate(result):
            treev_view_search_window.insert(
                "", END, text=order[0], values=(index + 1, *order[1:])
            )
...
def update_game():
    ...
    genre = entry_genre_add_update_delete_window.get().strip()
    age_range = entry_age_range_add_update_delete_window.get().strip()
    result = connection.update_game(
        name, company, date, price, genre, age_range, new_name
    )
    if result[0] == -1:
        msb.showwarning("Error", result[1])
    elif result[0] == 0:
...
def search_between_games():
    ...
    if age_range == "":
        age_range = None
    result = connection.search_between_games(
        name, company, date, price_min, price_max, genre, age_range
    )
    treev_view_search_window.delete(*treev_view_search_window.get_children())
    for index, order in enumerate(result):
        treev_view_search_window.insert(
...
root = Tk()
root.config(bg=BG)
btn_sign_up_root = Button(
    root,
    cnf=CNF_BTNS,
    text="Sign Up",
    command=lambda: change_window(root, sign_up_window),
)
btn_login_root = Button(
    root, cnf=CNF_BTNS, text="Login", command=lambda: change_window(root, login_window)
)
...
label_password2_sign_up_window = Label(
    sign_up_window, text="Repeat Password: ", cnf=CNF_LBLS
)
...
btn_sign_up_sign_up_window = Button(
    sign_up_window, cnf=CNF_BTNS_SIGN_UP_WINDOW, text="Sign Up", command=create_account
)
btn_back_sign_up_window = Button(
    sign_up_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    text="Back",
    command=lambda: change_window(sign_up_window, root),
)
...
btn_login_login_window = Button(
    login_window, cnf=CNF_BTNS_SIGN_UP_WINDOW, text="Login", command=login
)
btn_back_login_window = Button(
    login_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    text="Back",
    command=lambda: change_window(login_window, root),
)
...
btn_view_orders_main_window_manager = Button(
    main_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="View Orders",
    command=lambda: change_window(main_window, view_orders_window_manager),
)
btn_view_orders_main_window_user = Button(
    main_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="View Orders",
    command=lambda: change_window(main_window, view_orders_window_user),
)
btn_add_update_delete_game_main_window = Button(
    main_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Add/Update/Delete a Game",
    command=lambda: change_window(main_window, add_update_delete_window),
)
btn_search_games_main_window = Button(
    main_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Search Games",
    command=lambda: change_window(main_window, search_window),
)
btn_logout_main_window = Button(
    main_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Logout",
    command=lambda: change_window(main_window, login_window),
)
...
label_price_min_search_window = Label(
    search_window, text="Minimum Price: ", cnf=CNF_LBLS
)
label_price_max_search_window = Label(
    search_window, text="Maximum Price: ", cnf=CNF_LBLS
)
...
btn_search_search_window = Button(
    search_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Search Games",
    command=search_between_games,
)
btn_order_search_window = Button(
    search_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Order Game",
...
btn_back_search_window = Button(
    search_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Back",
    command=lambda: change_window(search_window, main_window),
)
...
treev_view_search_window = ttk.Treeview(
    search_window, selectmode="browse", show="headings"
)
treev_view_search_window.grid(row=1, rowspan=9, column=3, sticky="ns")
verscrlbar_treev_view_search_window = ttk.Scrollbar(
    search_window, orient="vertical", command=treev_view_search_window.yview
)
verscrlbar_treev_view_search_window.grid(row=1, rowspan=9, column=4, sticky="ns")
treev_view_search_window.config(yscrollcommand=verscrlbar_treev_view_search_window.set)
...
label_name_add_update_delete_window = Label(
    add_update_delete_window, text="Name: ", cnf=CNF_LBLS
)
label_new_name_add_update_delete_window = Label(
    add_update_delete_window, text="New Name: ", cnf=CNF_LBLS
)
label_company_add_update_delete_window = Label(
    add_update_delete_window, text="Company: ", cnf=CNF_LBLS
)
label_date_add_update_delete_window = Label(
    add_update_delete_window, text="Date:", cnf=CNF_LBLS
)
label_price_add_update_delete_window = Label(
    add_update_delete_window, text="Price: ", cnf=CNF_LBLS
)
label_genre_add_update_delete_window = Label(
    add_update_delete_window, text="Genre: ", cnf=CNF_LBLS
)
label_age_range_add_update_delete_window = Label(
    add_update_delete_window, text="Age Range: ", cnf=CNF_LBLS
)
...
entry_age_range_add_update_delete_window = Entry(
    add_update_delete_window, cnf=CNF_ENTRY
)
btn_insert_add_update_delete_window = Button(
    add_update_delete_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Insert Game",
...
btn_update_add_update_delete_window = Button(
    add_update_delete_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Update Game",
...
btn_delete_add_update_delete_window = Button(
    add_update_delete_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Delete Game",
...
btn_reset_add_update_delete_window = Button(
    add_update_delete_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Reset",
...
btn_back_add_update_delete_window = Button(
    add_update_delete_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Back",
    command=lambda: change_window(add_update_delete_window, main_window),
)
btn_search_add_update_delete_window = Button(
    add_update_delete_window,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Search",
...
############################## End add_update_delete_window ##############################


############################## view_orders_window_manager ##############################
...
btn_back_view_orders_window_manager = Button(
    view_orders_window_manager,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Back",
    command=lambda: change_window(view_orders_window_manager, main_window),
)
btn_back_view_orders_window_manager.grid(
    row=2, column=1, cnf=CNF_BTNS_SIGN_UP_WINDOW_GRID
)
treev_view_orders_window_manager = ttk.Treeview(
    view_orders_window_manager, selectmode="browse", show="headings"
)
treev_view_orders_window_manager.grid(row=1, column=1)
verscrlbar_treev_view_orders_window_manager = ttk.Scrollbar(
    view_orders_window_manager,
    orient="vertical",
    command=treev_view_orders_window_manager.yview,
)
verscrlbar_treev_view_orders_window_manager.grid(row=1, column=2, sticky="ns")
treev_view_orders_window_manager.config(
    yscrollcommand=verscrlbar_treev_view_orders_window_manager.set
)
...
############################## End view_orders_window_manager ##############################


############################## view_orders_window_user ##############################
...
btn_back_view_orders_window_user = Button(
    view_orders_window_user,
    cnf=CNF_BTNS_SIGN_UP_WINDOW,
    width=24,
    text="Back",
    command=lambda: change_window(view_orders_window_user, main_window),
)
btn_back_view_orders_window_user.grid(row=2, column=1, cnf=CNF_BTNS_SIGN_UP_WINDOW_GRID)
treev_view_orders_window_user = ttk.Treeview(
    view_orders_window_user, selectmode="browse", show="headings"
)
treev_view_orders_window_user.grid(row=1, column=1)
verscrlbar_treev_view_orders_window_user = ttk.Scrollbar(
    view_orders_window_user,
    orient="vertical",
    command=treev_view_orders_window_user.yview,
)
verscrlbar_treev_view_orders_window_user.grid(row=1, column=2, sticky="ns")
treev_view_orders_window_user.config(
    yscrollcommand=verscrlbar_treev_view_orders_window_user.set
)
